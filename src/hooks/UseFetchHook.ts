...
const fetchFromPro = async (prompt) => {
  ...
  try {
    ...
    response =
      result.response.candidates[0].finishReason.toUpperCase() === "STOP"
        ? result.response.candidates[0].content?.parts
        : [{ text: "Something went wrong", err: "YES" }];
  } catch (error) {
    return error;
  }
...
const fetchFromVision = async (prompt) => {
  ...
  let promptExtra =
    "If this image contains coding quesions including database also provide only the code no description needed. Line Number not needed";
...
const fetchData = async (getPrompt) => {
  ...
  if (prompt.usage_flag === false) {
    const promptExtra =
      prompt.data + " provide only the code no description needed.";
